...
public class PositionReader : MonoBehaviour {
    ...
    private void Update()
    {
        for (int i = 0; i < accelerometer.Length; i++)
        {
            ...
            pitch[i] = 180 * Mathf.Atan(accelerationX / Mathf.Sqrt(accelerationY * accelerationY + accelerationZ * accelerationZ)) / Mathf.PI;
            roll[i] = 180 * Mathf.Atan(accelerationY / Mathf.Sqrt(accelerationX * accelerationX + accelerationZ * accelerationZ)) / Mathf.PI;
        }
